...
export async function translateConversationImage(
  ...
  const contentGroups = allGroups.filter((group) => group.bbox.top >= contentBottom * 0.08);
  const groups = contentGroups.filter((group, index) => !isLikelySenderLabel(group, contentGroups[index + 1]));
  ...
  for (const [index, group] of groups.entries()) {
    ...
    if (side === "incoming") {
      ...
      if (isReliablePartnerCandidate(sourceText, translation.sourceLanguage)) {
    ...
    if (side === "outgoing") {
      const correction = await correctChat(sourceText, {
...
export function groupOcrLines(lines: OcrLine[]): OcrMessageGroup[] {
  ...
  const center = maxRight / 2;
  const spread = Math.max(maxRight * 0.1, 30);
  ...
  for (const line of positioned) {
    ...
    const closeEnough = previous && gap <= Math.max(line.bbox.height, lastLineHeights.at(-1) ?? line.bbox.height) * 1.5;
    const aligned = previous && Math.abs(previous.bbox.left - line.bbox.left) <= Math.max(44, maxRight * 0.12);
    if (previous && sameSide && closeEnough && aligned) {
      ...
      if (previous.side === "unknown") previous.side = line.side;
...
async function recognizeImage(image: Buffer, config: ReturnType<typeof imageBridgeConfig>) {
  ...
  const recognition = worker.recognize(image, { rotateAuto: true }, { blocks: true });
...
function sideFromBbox(bbox: OcrLine["bbox"], center: number, spread: number): ImageMessageSide {
  ...
  if (messageCenter < center - spread) return "incoming";
  if (messageCenter > center + spread) return "outgoing";
  return "unknown";
}
...
function isOcrMetadata(text: string) {
  ...
  return /TalkBridge|AI채팅|번역채팅|상대언어|내언어|자동감지|번역미리보기|번역켜짐|번역끄기|번역됨|상대에게.*전송|^오늘\d|^\d{2,4}$|^\d{1,2}:\d{2}$|^오전\d{1,2}:\d{2}$|^오후\d{1,2}:\d{2}$/iu.test(compact);
}
...
function isLikelySenderLabel(group: OcrMessageGroup, next?: OcrMessageGroup) {
  if (!next || group.side !== "incoming" || next.side !== "incoming") return false;
